<?php
// kpsc_recruitments.php
// Dynamically fetch KPSC Recruitments for the current year
    ...
    foreach ($links as $link) {
        ...
        if (strpos($text, (string)$current_year) !== false) {
            if (strpos($href, 'http') !== 0) {
                $href = 'https://kpsc.kar.nic.in' . (strpos($href, '/') === 0 ? '' : '/') . $href;
            }
            $is_duplicate = false;
            foreach($jobs as $job) {
...
<head>
    ...
    <title>KPSC Live Recruitments</title>
    ...
    <div class="container">
        ...
        <?php if(empty($jobs)): ?>
            <div class="empty-state">
                ...
                <p>There are currently no new recruitment updates on KPSC for <?php echo $current_year; ?>.</p>
                <a href="<?php echo $url; ?>" target="_blank" class="apply-btn" style="display: inline-block; margin-top: 1.5rem;">Visit KPSC Official Site</a>
            </div>
        <?php else: 
            $active_jobs = [];
            $expired_jobs = [];
        ...
        ?>
            <p style="color: #64748b; margin-bottom: 1.5rem; font-size: 0.95rem;">
                Successfully fetched <strong><?php echo count($jobs); ?></strong> live updates from KPSC.
            </p>
            ...
            <?php foreach($active_jobs as $job): ?>
                <div class="job-card">
                    <div>
                        ...
                        <div class="job-meta" style="margin-bottom: 5px;">📍 Karnataka Public Service Commission</div>
