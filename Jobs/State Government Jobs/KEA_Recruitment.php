<?php
// kea_recruitments.php
// Dynamically fetch KEA Recruitments for the current year
...
if ($html) {
    ...
    // Find all links
    $links = $xpath->query('//a');
    $current_group_year_match = false;
    $current_group_title = "";
    ...
    foreach ($links as $link) {
        ...
        // Identify dead/header links used for dropdowns
        $is_header = ($href_lower == '' || 
                      $href_lower == '#' || 
                      $href_lower == 'https://cetonline.karnataka.gov.in/kea/' ||
                      $href_lower == 'https://cetonline.karnataka.gov.in/kea' ||
                      $href_lower == '/kea/' ||
                      $href_lower == '/kea' ||
                      strpos($href_lower, 'indexnew.aspx') !== false);
        
        if (strpos($text, (string)$current_year) !== false) {
            $current_group_year_match = true;
            $current_group_title = preg_replace('/\s*-\s*\d{4}/', '', $text);
        } elseif ($is_header) {
            $current_group_year_match = false;
            $current_group_title = "";
        }
        
        $is_match = (strpos($text, (string)$current_year) !== false) || 
                    ($current_group_year_match && preg_match('/\(RPC\)|\(KK\)|\(HK\)|\(NON-HK\)/i', $text));

        // Filter: Must contain the current year and be a recruitment/job, and not be a dead header link
        if ($is_match && !$is_header) {
            $lower = strtolower($text);
            
            // Append parent title for child links (like RPC/KK) so they are descriptive
            if (strpos($text, (string)$current_year) === false && $current_group_title != "") {
                $text = $current_group_title . " ➔ " . $text;
            }
            
            // Exclude admission-related keywords
            if (strpos($lower, 'cet') === false && 
                strpos($lower, 'neet') === false && 
                strpos($lower, 'seat allotment') === false &&
                strpos($lower, 'option entry') === false &&
                strpos($lower, 'verification') === false &&
                strpos($lower, 'mock') === false &&
                strpos($lower, 'college') === false &&
                strpos($lower, 'admission') === false) {
                
                // Make relative URLs absolute
                if (strpos($href, 'http') !== 0) {
                    // KEA links are relative to /kea/
                    $href = 'https://cetonline.karnataka.gov.in/kea' . (strpos($href, '/') === 0 ? '' : '/') . $href;
                }
                
                // Avoid duplicates
                $is_duplicate = false;
                foreach($jobs as $job) {
                    if($job['link'] == $href || $job['title'] == $text) {
                ...
                if (!$is_duplicate) {
                    $jobs[] = [
                        ...
                        'link' => htmlspecialchars($href),
                        'date' => fetch_latest_date($href)
                    ];
                }
            }
        }
...
<head>
    ...
    <title>KEA Live Recruitments</title>
    ...
    <div class="container">
        ...
        <?php if(empty($jobs)): ?>
            <div class="empty-state">
                ...
                <p>There are currently no new recruitment updates on KEA for <?php echo $current_year; ?>.</p>
                <a href="https://cetonline.karnataka.gov.in/kea/" target="_blank" class="apply-btn" style="display: inline-block; margin-top: 1.5rem;">Visit KEA Official Site</a>
            </div>
        <?php else: 
            $active_jobs = [];
            $expired_jobs = [];
        ...
        ?>
            <p style="color: #64748b; margin-bottom: 1.5rem; font-size: 0.95rem;">
                Successfully fetched <strong><?php echo count($jobs); ?></strong> live updates from KEA.
            </p>
            ...
            <?php foreach($active_jobs as $job): ?>
                <div class="job-card">
                    <div>
                        ...
                        <div class="job-meta" style="margin-bottom: 5px;">📍 Karnataka Examination Authority</div>
