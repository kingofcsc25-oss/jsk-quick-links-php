document.addEventListener('DOMContentLoaded', async () => {
    ...
    const email = urlParams.get('email') || 'agent@karnataka.gov.in';
    
    // User Provided Credentials
    const key = 'FeB6HB';
    const salt = 'bX8eWBUjT18v8JbGWoK8rObVVfsKJqKx';
    const txnid = 'SSPCM_' + Date.now() + Math.floor(Math.random() * 1000);
    const productinfo = 'WalletRecharge';
    
    const surl = 'https://ssp.postmatric.karnataka.gov.in/dl-payu-success';
    const furl = 'https://ssp.postmatric.karnataka.gov.in/dl-payu-failure';
